...
def create_output_creation_program() -> ArgumentParser:
	...
	group_output_creation.add_argument('--frame-interpolator-target-fps', help = translator.get('help.frame_interpolator_target_fps'), type = float, default = config.get_float_value('output_creation', 'frame_interpolator_target_fps'))
	job_store.register_step_keys([ 'output_image_quality', 'output_image_scale', 'output_audio_encoder', 'output_audio_quality', 'output_audio_volume', 'output_video_encoder', 'output_video_preset', 'output_video_quality', 'output_video_scale', 'output_video_fps', 'frame_interpolator_target_fps' ])
	return program
...
def create_program() -> ArgumentParser:
	...
	doctor_program = sub_program.add_parser('doctor', help = translator.get('help.doctor'), parents = [ create_config_path_program(), create_temp_path_program(), create_jobs_path_program(), create_execution_program(), create_log_level_program() ], formatter_class = create_help_formatter_large)
	doctor_program.add_argument('--verify-models', help = translator.get('help.verify_models'), action = 'store_true', default = False)
