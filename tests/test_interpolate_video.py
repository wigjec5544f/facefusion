...
def test_interpolate_frames_count() -> None:
	prev = numpy.zeros((4, 4, 3), dtype = numpy.uint8)
	nxt = numpy.zeros((4, 4, 3), dtype = numpy.uint8)
	with mock.patch.object(interpolate_video.frame_interpolator, 'interpolate_pair', side_effect = lambda a, b, timestep: numpy.full_like(a, int(timestep * 100))):
		intermediates = interpolate_video.interpolate_frames(prev, nxt, multiplier = 4)
...
def test_interpolate_frames_multiplier_two() -> None:
	...
	with mock.patch.object(interpolate_video.frame_interpolator, 'interpolate_pair', return_value = numpy.zeros((2, 2, 3), dtype = numpy.uint8)) as patched:
		interpolate_video.interpolate_frames(prev, nxt, multiplier = 2)
